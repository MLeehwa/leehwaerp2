...
import { PlusOutlined, EditOutlined, DeleteOutlined, CarOutlined, PrinterOutlined } from '@ant-design/icons'
...
const AdminAssets = () => {
  ...
  useEffect(() => {
    fetchAssets()
  }, [filterStatus, filterCategory, filterCompany, searchTerm])
  ...
  const handleEdit = async (asset: Asset) => {
    form.setFieldsValue({
      ...asset,
      company: asset.company?._id,
      purchaseDate: asset.purchaseDate ? dayjs(asset.purchaseDate) : undefined,
      warrantyExpiryDate: asset.warrantyExpiryDate ? dayjs(asset.warrantyExpiryDate) : undefined,
    })
  ...
  const handleExport = () => {
    // 관리대장 출력 기능 (추후 구현)
    message.info('관리대장 출력 기능은 추후 구현 예정입니다')
  }
  ...
  const handleSubmit = async (values: any) => {
    try {
      ...
      message.error(error.response?.data?.message || '자산 저장에 실패했습니다')
    }
  }
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 16 }}>
        <h1>총무 자산 관리</h1>
        <Space>
          <Button icon={<PrinterOutlined />} onClick={handleExport}>
            관리대장 출력
          </Button>
      ...
      >
        ...
        >
          ...
          <Form.Item name="managedBy" hidden>
            <Input />
          </Form.Item>
